...
def cansel(message):
    bot.send_message(message.chat.id, "Чтобы посмотреть команды, используй❤️❤️❤️ - /info", reply_markup=hideBoard)
...
def start_command(message):
    bot.send_message(message.chat.id, """Привет! Я бот для проектов❤️
...
"""
Вот команды которые помогут тебе❤️❤️❤️:

/new_project - используй для добавления нового проекта
/projects - показывает все проекты
....

А ещё ты можешь ввести имя проекта и узнать информацию о нем!""")
    

@bot.message_handler(commands=['new_project'])
def addtask_command(message):
    bot.send_message(message.chat.id, "Введи название проекта😘:")
...
def link_project(message, data):
    ...
    bot.send_message(message.chat.id, "Введи текущий статус проекта", reply_markup=gen_markup(statuses))
...
def callback_project(message, data, statuses):
    ...
    bot.send_message(message.chat.id, "Я сохранил твой проект🤞")
...
def skill_handler(message):
    ...
    if projects:
        ...
        bot.send_message(message.chat.id, 'Выбери проект для которого нужно выбрать навык ☺️', reply_markup=gen_markup(projects))
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'У тебя ещё нет такого проекта, попробуй еще раз!) Выбери проект для которого нужно выбрать навык', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
        skills = [x[1] for x in manager.get_skills()]
        bot.send_message(message.chat.id, 'Выбери навык ☺️', reply_markup=gen_markup(skills))
